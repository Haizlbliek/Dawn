...
namespace Dawn {
	static class TerrainController {
		...
		private static void InTexture(in DawnRoomSettings settings, in Time time, out TerrainPalette.PaletteInfo mainPalette, out TerrainPalette.PaletteInfo fadePalette, out float fade) {
			...
			if (timeSettings.terrainFadePalette?.fades?.Length <= Plugin.instance.currentCameraPosition) {
				fade = 0.0f;
			}
			else {
				fade = timeSettings.terrainFadePalette?.fades[Plugin.instance.currentCameraPosition] ?? 0.0f;
			}
		}
		...
		private static void On_TerrainPalette_UpdateFade(On.TerrainPalette.orig_UpdateFade orig, TerrainPalette self, float fade, float mushroom, float rain, float echo, float rot) {
			if (Plugin.instance.currentRoom.roomSettings is not DawnRoomSettings settings) {
			...
			InTexture(settings, Plugin.instance.timeLerpA, out TerrainPalette.PaletteInfo paletteAMain, out TerrainPalette.PaletteInfo paletteAFade, out float fadeA);
			InTexture(settings, Plugin.instance.timeLerpB, out TerrainPalette.PaletteInfo paletteBMain, out TerrainPalette.PaletteInfo paletteBFade, out float fadeB);
			...
			// Plugin.instance.Log("Palettes: A = " + (paletteAMain?.name ?? "null") + "-" + (paletteAFade?.name ?? "null") + "; B = " + (paletteBMain?.name ?? "null") + "-" + (paletteBFade?.name ?? "null"));

			if (Plugin.instance.lerpAmount <= 0.01f || (paletteAMain?.name == paletteBMain?.name && paletteAFade?.name == paletteBFade?.name)) {
				// Plugin.instance.Log("A");
			...
			else if (Plugin.instance.lerpAmount >= 0.99f) {
				// Plugin.instance.Log("B");
				...
				// Plugin.instance.Log("C - " + Dawn.instance.lerpAmount);
				self.texturePixels = new Color[(paletteAMain?.PaletteSize.x * paletteAMain?.PaletteSize.y) ?? 0];
				self.fadePixels = new Color[(paletteAFade?.PaletteSize.x * paletteAFade?.PaletteSize.y) ?? 0];
				LerpBetween(paletteAMain, paletteAFade, self.texturePixels, self.fadePixels, fadeA, rain, echo);
				...
				TerrainPalette.LerpColors(self.texturePixels, self.fadePixels, Plugin.instance.lerpAmount);
			}
			...
			RoomSettings timeA = settings.GetTimeSetting(Plugin.instance.timeLerpA);
			RoomSettings timeB = settings.GetTimeSetting(Plugin.instance.timeLerpB);
			settings.terrainLight = Mathf.Lerp(timeA.terrainLight ?? time_.terrainLight ?? DefaultRoomSettings.ancestor.TerrainLight, timeB.terrainLight ?? time_.terrainLight ?? DefaultRoomSettings.ancestor.TerrainLight, Plugin.instance.lerpAmount);
			settings.terrainStainAmount = Mathf.Lerp(timeA.terrainStainAmount ?? time_.terrainStainAmount ?? DefaultRoomSettings.ancestor.TerrainStainAmount, timeB.terrainStainAmount ?? time_.terrainStainAmount ?? DefaultRoomSettings.ancestor.TerrainStainAmount, Plugin.instance.lerpAmount);
			settings.terrainStainBrightness = Mathf.Lerp(timeA.terrainStainBrightness ?? time_.terrainStainBrightness ?? DefaultRoomSettings.ancestor.TerrainStainBrightness, timeB.terrainStainBrightness ?? time_.terrainStainBrightness ?? DefaultRoomSettings.ancestor.TerrainStainBrightness, Plugin.instance.lerpAmount);
			settings.terrainStainHeight = Mathf.Lerp(timeA.terrainStainHeight ?? time_.terrainStainHeight ?? DefaultRoomSettings.ancestor.TerrainStainHeight, timeB.terrainStainHeight ?? time_.terrainStainHeight ?? DefaultRoomSettings.ancestor.TerrainStainHeight, Plugin.instance.lerpAmount);
			settings.terrainGooHeight = Mathf.Lerp(timeA.terrainGooHeight ?? time_.terrainGooHeight ?? DefaultRoomSettings.ancestor.TerrainGooHeight, timeB.terrainGooHeight ?? time_.terrainGooHeight ?? DefaultRoomSettings.ancestor.TerrainGooHeight, Plugin.instance.lerpAmount);
			settings.terrainGrain = Mathf.Lerp(timeA.terrainGrain ?? time_.terrainGrain ?? DefaultRoomSettings.ancestor.TerrainGrain, timeB.terrainGrain ?? time_.terrainGrain ?? DefaultRoomSettings.ancestor.TerrainGrain, Plugin.instance.lerpAmount);
			settings.terrainEdgeRadius = Mathf.Lerp(timeA.terrainEdgeRadius ?? time_.terrainEdgeRadius ?? DefaultRoomSettings.ancestor.TerrainEdgeRadius, timeB.terrainEdgeRadius ?? time_.terrainEdgeRadius ?? DefaultRoomSettings.ancestor.TerrainEdgeRadius, Plugin.instance.lerpAmount);
			settings.terrainWaves = Mathf.Lerp(timeA.terrainWaves ?? time_.terrainWaves ?? DefaultRoomSettings.ancestor.TerrainWaves, timeB.terrainWaves ?? time_.terrainWaves ?? DefaultRoomSettings.ancestor.TerrainWaves, Plugin.instance.lerpAmount);
			settings.terrainDepth = Mathf.Lerp(timeA.terrainDepth ?? time_.terrainDepth ?? DefaultRoomSettings.ancestor.TerrainDepth, timeB.terrainDepth ?? time_.terrainDepth ?? DefaultRoomSettings.ancestor.TerrainDepth, Plugin.instance.lerpAmount);
			settings.terrainSkyFade = Mathf.Lerp(timeA.terrainSkyFade ?? time_.terrainSkyFade ?? DefaultRoomSettings.ancestor.TerrainSkyFade, timeB.terrainSkyFade ?? time_.terrainSkyFade ?? DefaultRoomSettings.ancestor.TerrainSkyFade, Plugin.instance.lerpAmount);
		}
	}
}